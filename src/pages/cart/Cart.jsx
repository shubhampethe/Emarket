...
import { addDoc, collection, doc,query,orderBy ,onSnapshot,deleteDoc} from 'firebase/firestore';
...
function Cart() {
  ...
  const { mode,id,cartProuct} = context;
...
// updated code
  ...
  const deleteCart = async(item) => {
    auth.onAuthStateChanged(user=>{
      if(user){
        const productRef = collection(fireDB, "Cart" +user.uid);
  ...
  return (
    ...
    <Layout >
      ...
      (
        <h2 className=' text-center tex-2xl'>No Cart items are available</h2>
      )
}
    </Layout>
